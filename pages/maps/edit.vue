...
//       this.$store.dispatch('mapEdit/mapSocket', {...hoge, method: 'update'})
    ...
    socket.on('layer/add', layer => {
      this.$store.dispatch('mapEdit/layerSocket', {
        layer: layer,
        method: 'add',
      });
    });
    ...
    socket.on('select/clear', prop => {
      this.$store.dispatch('mapEdit/selectSocket', {
        ...prop,
        method: 'clear',
      });
    });
    ...
    this.$axios
      ...
      .then(response => {
        ...
        if (!map.layer || !map.layer.length) {
          // init layer
          this.layerSvcCreate(mapId, '新しいレイヤー', '', 'red')
            .then(response => {
          ...
          map.layers.forEach(layer =>
            this.$store.commit('mapEdit/updateLayer', layer)
          );
          this.$store.commit(
            'mapEdit/selectLayer',
