...
const mutations = {
  ...
  setLike(state, user_id) {
    state.liked =
      state.map.likes.filter(function(element) {
  ...
  changeLike(state) {
    if (state.liked) {
      this.$axios.delete(
        process.env.API_URL + '/maps/' + state.map.id + '/like'
      );
    } else {
      this.$axios.post(
        process.env.API_URL + '/maps/' + state.map.id + '/like',
  ...
  changeStock(state) {
    if (state.stocked) {
      this.$axios.delete(
        process.env.API_URL + '/maps/' + state.map.id + '/stock'
      );
    } else {
      this.$axios.post(
        process.env.API_URL + '/maps/' + state.map.id + '/stock',
  ...
  setStock(state, user_id) {
    state.stocked =
      state.map.stocks.filter(function(element) {
  ...
  addTool(state, { tool, layerId }) {
    ...
    state.layers.find(layer => layer.id === layerId).tools = {
      ...tools,
      [tool.id]: tool,
    };
  },
  ...
  addComment(state, { prop, getters }) {
    if (!['pin', 'box'].includes(state.activeLayer.tools[prop.toolId].type))
      return;
  ...
  replot(state, prop) {
    state.activeLayer.tools[prop.toolId].points.splice(
      prop.index,
      1,
      prop.point
    );
  ...
  replotAll(state, prop) {
    ...
    state.activeLayer.tools[prop.toolId].points.forEach((point, index) => {
      state.activeLayer.tools[prop.toolId].points.splice(index, 1, {
        lat: point.lat + dlat,
        lng: point.lng + dlng,
      });
    });
  ...
  resize(state, prop) {
    ...
    state.activeLayer.tools[prop.toolId] = {
      ...tool,
      width: prop.width,
      height: prop.height,
    };
  ...
  moveTools(state, { prop, getters }) {
    ...
    for (const toolId of Object.keys(getters.selecting())) {
      ...
      state.activeLayer.tools[toolId] = {
        ...tool,
        lat: tool.lat + dlat,
        lng: tool.lng + dlng,
      };
...
const actions = {
  ...
  selectTool(context, prop) {
    ...
    if (!prop.multiple) context.dispatch('clearSelection');
  ...
  selectSocket(context, prop) {
    ...
    switch (prop.method) {
      ...
      case 'clear':
        context.commit(
          'clearSelection',
          context.getters.othersSelecting(userId)
        );
        break;
    }
  },
...
const getters = {
  ...
  comments(state, _, rootState) {
    const toolsWithComments = state.layers
      .map(layer => {
        return Object.values(layer.tools).filter(tool =>
          ['pin', 'box'].includes(tool.type)
        );
      })
      .flat(10)
      .filter(tool => tool.comments.length);

    const comments = toolsWithComments
      .map(tool => {
